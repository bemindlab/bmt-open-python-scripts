...
def get_current_commit() -> CommitInfo:
    ...
    date = subprocess.check_output(["git", "log", "-1", "--pretty=format:%ad"]).decode().strip()
    message = subprocess.check_output(["git", "log", "-1", "--pretty=format:%B"]).decode().strip()
...
def get_changed_files() -> List[str]:
    ...
    try:
        output = subprocess.check_output(["git", "diff", "--cached", "--name-only"]).decode().strip()
        return output.split("\n") if output else []
    except subprocess.CalledProcessError:
        return []
...
def save_summary(summary: str, commit_info: CommitInfo, changed_files: List[str]) -> str:
