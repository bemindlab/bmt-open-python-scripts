...
def run(script):
    ...
    if len(parts) != 2:
        console.print(
            f"[red]รูปแบบชื่อสคริปต์ไม่ถูกต้อง กรุณาระบุในรูปแบบ 'หมวดหมู่.ชื่อสคริปต์'[/red]"
        )
        return
    ...
    try:
        ...
        # ตั้งค่า PYTHONPATH ให้รวมถึงโฟลเดอร์หลักของโปรเจค
        project_root = find_project_root()
        if project_root:
            env = os.environ.copy()
            # เพิ่มโฟลเดอร์หลักของโปรเจคเข้าไปใน PYTHONPATH
            if "PYTHONPATH" in env:
                env["PYTHONPATH"] = f"{project_root}:{env['PYTHONPATH']}"
            else:
                env["PYTHONPATH"] = str(project_root)
            
            # รันสคริปต์ด้วย PYTHONPATH ที่ตั้งค่าแล้ว
            subprocess.run([sys.executable, script_path] + args, check=True, env=env)
        else:
            # ถ้าไม่พบโฟลเดอร์หลักของโปรเจค ให้รันสคริปต์ตามปกติ
