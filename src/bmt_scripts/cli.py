...
def version():
    ...
    try:
        ...
        # แสดงข้อมูลระบบ
        system_info = Table(title="ข้อมูลระบบ", show_header=True, header_style="bold magenta")
        ...
        system_info.add_row("ระบบปฏิบัติการ", f"{platform.system()} {platform.release()}")
...
def webcam_show(camera_id, width, height):
    ...
    try:
        ...
        print_error("ไม่พบโมดูลที่จำเป็น กรุณาติดตั้งด้วยคำสั่ง: pip install 'bmt-scripts[hardware]'")
...
def webcam_list():
    ...
    try:
        ...
        table = Table(title="รายการกล้อง", show_header=True, header_style="bold magenta")
...
@click.option("--output-dir", "-o", default="screen_captures", help="ไดเรกทอรีสำหรับบันทึกภาพ")
@click.option("--filename", "-f", help="ชื่อไฟล์ (ถ้าไม่ระบุจะใช้วันที่และเวลา)")
@click.option("--region", "-r", help="พื้นที่ที่ต้องการจับภาพ (รูปแบบ: x,y,width,height)")
    ...
    try:
        ...
        if region:
            try:
                ...
                image_path = screen.capture_region(x, y, width, height, os.path.join(output_dir, filename))
            except ValueError:
                print_error("รูปแบบพื้นที่ไม่ถูกต้อง กรุณาระบุในรูปแบบ: x,y,width,height")
                return
        else:
...
def screen_info():
    ...
    try:
        ...
        table = Table(title="ข้อมูลหน้าจอ", show_header=True, header_style="bold magenta")
        table.add_column("รหัส", style="cyan")
...
def git_commit_summary(repo_path, days, author):
    ...
    try:
        ...
        with Progress(
            ...
            progress.add_task(description="กำลังวิเคราะห์ Git repository...", total=None)
            time.sleep(1)  # แสดงการโหลด
        ...
        print_error("ไม่พบโมดูลที่จำเป็น กรุณาติดตั้งด้วยคำสั่ง: pip install 'bmt-scripts[dev]'")
    except Exception as e:
        print_error(f"เกิดข้อผิดพลาด: {e}")
...
@click.option("--output-dir", "-o", default="mockups", help="ไดเรกทอรีสำหรับบันทึก mockup")
def agents_mockup(prompt, output_dir):
    ...
    try:
        ...
        print_error("ไม่พบโมดูลที่จำเป็น กรุณาติดตั้งด้วยคำสั่ง: pip install 'bmt-scripts[agents]'")
    except Exception as e:
        print_error(f"เกิดข้อผิดพลาด: {e}")
...
@click.option("--type", "-y", default="article", type=click.Choice(['article', 'script', 'ideas']), 
...
def autogen_creative(topic, type, output_file):
    ...
    try:
        ...
        # แสดงผลลัพธ์
        console.print(Panel.fit(result, title=f"เนื้อหา{type}ที่สร้าง", border_style="cyan"))
    except ImportError:
        print_error("ไม่พบโมดูลที่จำเป็น กรุณาติดตั้งด้วยคำสั่ง: pip install 'bmt-scripts[agents]'")
    except Exception as e:
        print_error(f"เกิดข้อผิดพลาด: {e}")
...
def run(script):
    ...
    parts = script.split(".")
    if len(parts) != 2:
        print_error("รูปแบบชื่อสคริปต์ไม่ถูกต้อง กรุณาระบุในรูปแบบ 'category.script-name'")
        return
...
def check_dependencies():
    ...
    try:
        ...
        # สร้างตารางแสดงผล
        table = Table(title="สถานะการติดตั้งแพ็คเกจ", show_header=True, header_style="bold magenta")
        ...
        print_info("  - ถ้าต้องการใช้ฟีเจอร์ hardware: pip install 'bmt-scripts[hardware]'")
